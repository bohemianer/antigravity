    ...
    localList.forEach(lItem => {
      ...
      if (!map.has(k)) {
        ...
        const mergedWord = {
          ...
          context: (lItem.context && lItem.context.length > (rItem.context || "").length) ? lItem.context : (rItem.context || lItem.context || ""),
          title: lItem.title || rItem.title || "Web Article",
          url: lItem.url || rItem.url || "",
          date: Math.max(lTime, rTime) || Date.now(),
          notes: lItem.notes ? (rItem.notes && lItem.notes !== rItem.notes ? `${rItem.notes}\n${lItem.notes}` : lItem.notes) : (rItem.notes || ""),
