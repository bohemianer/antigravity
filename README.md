...
| 🔍 划词悬浮查词卡 | 📋 Claude 风格笔记本 | 🎴 艾宾浩斯闪卡自测 |
|:---:|:---:|:---:|
| *(双击/划选单词触发)* | *(笔记本 + 统一菜单)* | *(翻转自测 + 键盘评级)* |

> 💡 **如想贡献截图**：欢迎在 Issues 中附上您的截图，帮助更多人了解产品！
...
5. 选择解压后得到的 `AntigravityVocabExtension` 文件夹（**直接选择该文件夹本身**，无需进入文件夹内部），点击「选择」即可完成安装！
   > 💡 安装成功后，浏览器右上角扩展栏会出现 Antigravity 图标。若未出现，请点击浏览器右上角扩展拼图图标 🧩 手动固定。
...
| **闪卡自测** | <kbd>2</kbd> | 标记为「🟠 初识阶段」（推进至下一复习阶段） |
...
- [x] **v1.0.0 (当前版本)**
  - [x] 外刊沉浸划词与智能语境例句捕获
  - [x] 词形还原查词（复数 / 时态 / 分词自动还原原形）
  - [x] 三路并发容灾翻译引擎（有道词典 + Google Dict + MyMemory 故障转移）
  - [x] IPA 音标清洗引擎与屈折词形音标智能合成
  - [x] 多词性自动分行与释义净化（过滤人名、精简前三义）
  - [x] 经典 Claude 暖色美学与全功能笔记本（行内无弹窗直接编辑）
  - [x] SM-2 艾宾浩斯自测闪卡系统与长效均衡调度算法
  - [x] VS Code / Outlook 双模职场摸鱼复习皮肤
  - [x] 全暗黑模式与跟随系统支持（popup + notebook 全覆盖）
  - [x] 坚果云 WebDAV 增量合并双向同步
  - [x] 欧路词典 (Eudic) OpenAPI 全量双向联动
  - [x] 新词自动置顶、老词修改保序的双轨排序逻辑
- [ ] **v1.1.0 (规划中)**
